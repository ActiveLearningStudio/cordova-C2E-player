...
var splitActivitypath = activityPath.split("/");
...
function onDeviceReady() {
      ...
      fileEntry.file(function (file) {
        ...
        reader.onloadend = function (evt) {
          ...
          window.requestFileSystem(
            ...
            function (fs) {
              window.resolveLocalFileSystemURL(
                `${fs.root.nativeURL}${activity_id[0]}`,
                (entry) => {
                  var reader = entry.createReader();
                  reader.readEntries((listProjects) => {
                    console.log(
                      "listProjects[0].nativeURL",
                      listProjects[0].nativeURL
                    );
                ...
                function (fileEntry) {
                  readLoacalJsonFile(fileEntry, (file) => {
                    ...
                    if (filterData && filterData.length) {
                      let title =
                        filterData[0].title && filterData[0].title["en-US"];
                      let response =
                        filterData[0].answer &&
                        filterData[0].answer["response"];
                      let newTitle;
                      if (title && title.includes("\n")) {
                        newTitle = title && title.split("\n");
                        console.log("else 1", newTitle);
                        newTitle =
                          newTitle[1] === "" ? newTitle : newTitle.shift();
                      } else {
                        newTitle = title;
                      }
                      ...
                      for (var i = 0; i < filterData.length; i++) {
                        if (filterData[i].title) {
                          html += `<div class="result-data">
                                            <div class="column"><p>${
                                              filterData[i].title &&
                                              filterData[i].title["en-US"]
                                            }</p></div>
                                            <div class="column"><p>${
                                              filterData[i].answer &&
                                              filterData[i].answer.response
                                            }</p></div>
                                            <div class="column">
                                                ${
                                                  filterData[i].answer.score &&
                                                  `<p>${filterData[i].answer.score.raw}/${filterData[i].answer.score.max}</p>`
                                                }
                                                ${
                                                  filterData[0].answer
                                                    .duration &&
                                                  `<p>${filterData[0].answer.duration}</p>`
                                                }
                      ...
                      html += `<button type="button" id="${
                        resultData.object &&
                        resultData.object.definition.extensions[
                          "http://h5p.org/x-api/h5p-local-content-id"
                        ]
                      }" class="btn green-btn1 remove-activityId">Retry</button>
                                </div> </div>`;
  ...
  $(document).on("click", ".remove-activityId", (evt) => {
    window.resolveLocalFileSystemURL(
      `file:///storage/emulated/0/Android/data/com.curriki.reader/cache/${activity_id[0]}-Offline.json`,
      function (fileEntry) {
        fileEntry.remove(
          function () {
  ...
  if (window.H5PIntegration) {
    ...
    H5P.externalDispatcher.on("xAPI", function (event) {
      ...
      if (
        ...
        event.data.statement.object.definition.description
      ) {
        let summary = {
      ...
      if (event.getVerb() === "completed") {
        ...
        retryActivity.addEventListener(
          "click",
          function () {
            window.resolveLocalFileSystemURL(
              `file:///storage/emulated/0/Android/data/com.curriki.reader/cache/${contentId}-Offline.json`,
              function (fileEntry) {
                fileEntry.remove(
                  function () {
                    console.log("File is removed.");
